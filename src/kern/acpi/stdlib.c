...
#ifndef uacpi_memcpy
void *
uacpi_memcpy (void *dest, const void *src, uacpi_size count)
{
	uacpi_char *cd = dest;
...
void *
uacpi_memmove (void *dest, const void *src, uacpi_size count)
{
	uacpi_char *cd = dest;
	const uacpi_char *cs = src;
...
#ifndef uacpi_memset
void *
uacpi_memset (void *dest, uacpi_i32 ch, uacpi_size count)
{
	uacpi_u8 fill = ch;
	uacpi_u8 *cdest = dest;
...
#ifndef uacpi_memcmp
uacpi_i32
uacpi_memcmp (const void *lhs, const void *rhs, uacpi_size count)
{
...
#ifndef uacpi_strlen
uacpi_size
uacpi_strlen (const uacpi_char *str)
{
...
uacpi_size
uacpi_strnlen (const uacpi_char *str, uacpi_size max)
{
	const uacpi_char *str1;
...
#ifndef uacpi_strcmp
uacpi_i32
uacpi_strcmp (const uacpi_char *lhs, const uacpi_char *rhs)
{
	uacpi_size i = 0;
...
void
uacpi_memcpy_zerout (void *dst, const void *src, uacpi_size dst_size,
										 uacpi_size src_size)
{
...
uacpi_u8
uacpi_bit_scan_forward (uacpi_u64 value)
{
...
uacpi_u8
uacpi_bit_scan_backward (uacpi_u64 value)
{
#if defined(_MSC_VER) && !defined(__clang__)
	unsigned char ret;
	unsigned long index;
...
void *
uacpi_builtin_alloc_zeroed (uacpi_size size)
{
	void *ptr;
...
static void
write_one (struct fmt_buf_state *fb_state, uacpi_char c)
{
...
static void
write_many (struct fmt_buf_state *fb_state, const uacpi_char *string,
						uacpi_size count)
{
	if (fb_state->bytes_written < fb_state->capacity)
		{
...
static uacpi_char
hex_char (uacpi_bool upper, uacpi_u64 value)
{
...
static void
write_padding (struct fmt_buf_state *fb_state, struct fmt_spec *fm,
...
static void
write_integer (struct fmt_buf_state *fb_state, struct fmt_spec *fm,
							 uacpi_u64 value)
{
...
static uacpi_bool
string_has_at_least (const uacpi_char *string, uacpi_size characters)
{
	while (*string)
		{
			if (--characters == 0)
...
static uacpi_bool
consume_digits (const uacpi_char **string, uacpi_size *out_size)
{
	uacpi_size size = 0;
...
static uacpi_bool
parse_number (const uacpi_char **fmt, enum parse_number_mode mode,
							uacpi_u64 *out_value)
{
	uacpi_status ret;
	...
	ret = uacpi_string_to_integer (digits, num_digits, UACPI_BASE_DEC,
																 out_value);
	return ret == UACPI_STATUS_OK;
}

static uacpi_bool
consume (const uacpi_char **string, const uacpi_char *token)
{
...
static uacpi_bool
is_one_of (uacpi_char c, const uacpi_char *list)
{
	for (; *list; list++)
		{
			if (c == *list)
...
static uacpi_bool
consume_one_of (const uacpi_char **string, const uacpi_char *list,
...
static uacpi_u32
base_from_specifier (uacpi_char specifier)
{
	switch (specifier)
		{
		case 'x':
...
static uacpi_bool
is_uppercase_specifier (uacpi_char specifier)
{
	return specifier == 'X';
}

static const uacpi_char *
find_next_conversion (const uacpi_char *fmt, uacpi_size *offset)
{
	*offset = 0;
...
uacpi_i32
uacpi_vsnprintf (uacpi_char *buffer, uacpi_size capacity,
	...
	while (*fmt)
		{
			...
			if (consume (&fmt, "ll")
					|| (sizeof (uacpi_size) == sizeof (long long)
							&& consume (&fmt, "z")))
				{
					if (consume (&fmt, "d") || consume (&fmt, "i"))
						{
...
uacpi_i32
uacpi_snprintf (uacpi_char *buffer, uacpi_size capacity, const uacpi_char *fmt,
								...)
{
	uacpi_va_list vlist;
	uacpi_i32 ret;
...
void
uacpi_log (uacpi_log_level lvl, const uacpi_char *str, ...)
{
