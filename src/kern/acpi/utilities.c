...
static uacpi_bool peek_one(const uacpi_char** str,
													 const uacpi_size* size,
													 uacpi_char* out_char) {
	if (*size == 0)
		return UACPI_FALSE;
...
static uacpi_bool consume_one(const uacpi_char** str,
															uacpi_size* size,
															uacpi_char* out_char) {
...
static uacpi_bool consume_if(const uacpi_char** str,
														 uacpi_size* size,
														 enum char_type type) {
	uacpi_char c;
...
static uacpi_bool consume_if_equals(const uacpi_char** str,
																		uacpi_size* size,
																		uacpi_char c) {
	uacpi_char c1;
...
uacpi_status uacpi_get_namespace_node_info(
	...
	if (obj->type == UACPI_OBJECT_DEVICE || obj->type == UACPI_OBJECT_PROCESSOR) {
		...
		if (uacpi_eval_dstate_method_template(node, dstate_method_template,
																					sizeof(sxd), sxd) == UACPI_STATUS_OK)
		...
		if (uacpi_eval_dstate_method_template(node, dstate_method_template,
																					sizeof(sxw), sxw) == UACPI_STATUS_OK)
...
static uacpi_iteration_decision find_one_device(void* opaque,
																								uacpi_namespace_node* node,
																								uacpi_u32 depth) {
