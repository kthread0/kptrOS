...
static uacpi_bool
is_char (uacpi_char c, enum char_type type)
{
	return (ascii_map[(uacpi_u8)c] & type) == type;
}

static uacpi_char
to_lower (uacpi_char c)
{
...
static uacpi_bool
peek_one (const uacpi_char **str, const uacpi_size *size, uacpi_char *out_char)
{
	if (*size == 0)
		return UACPI_FALSE;
...
static uacpi_bool
consume_one (const uacpi_char **str, uacpi_size *size, uacpi_char *out_char)
{
...
static uacpi_bool
consume_if (const uacpi_char **str, uacpi_size *size, enum char_type type)
{
	uacpi_char c;
...
static uacpi_bool
consume_if_equals (const uacpi_char **str, uacpi_size *size, uacpi_char c)
{
	uacpi_char c1;
...
uacpi_status
uacpi_string_to_integer (const uacpi_char *str, uacpi_size max_chars,
												 enum uacpi_base base, uacpi_u64 *out_value)
{
...
static inline uacpi_bool
is_valid_name_byte (uacpi_u8 c)
{
	// ‘_’ := 0x5F
	if (c == 0x5F)
		return UACPI_TRUE;
...
uacpi_bool
uacpi_is_valid_nameseg (uacpi_u8 *nameseg)
{
...
void
uacpi_eisa_id_to_string (uacpi_u32 id, uacpi_char *out_string)
{
...
uacpi_status
uacpi_eval_hid (uacpi_namespace_node *node, uacpi_id_string **out_id)
{
	uacpi_status ret;
...
void
uacpi_free_id_string (uacpi_id_string *id)
{
	if (id == UACPI_NULL)
		return;
...
uacpi_status
uacpi_eval_cid (uacpi_namespace_node *node, uacpi_pnp_id_list **out_list)
{
	uacpi_status ret;
				...
				default:
					{
						...
						uacpi_error (
								...
								name.text, i, uacpi_object_type_to_string (object->type));
						return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
					}
				}
		}
...
void
uacpi_free_pnp_id_list (uacpi_pnp_id_list *list)
{
	if (list == UACPI_NULL)
		return;
...
uacpi_status
uacpi_eval_sta (uacpi_namespace_node *node, uacpi_u32 *flags)
{
	uacpi_status ret;
	uacpi_u64 value = 0;
...
uacpi_status
uacpi_eval_adr (uacpi_namespace_node *node, uacpi_u64 *out)
{
...
static uacpi_u8
extract_package_byte_or_zero (uacpi_package *pkg, uacpi_size i)
{
	uacpi_object *obj;
...
uacpi_status
uacpi_eval_cls (uacpi_namespace_node *node, uacpi_id_string **out_id)
{
	uacpi_status ret;
	uacpi_object *obj;
	uacpi_package *pkg;
...
uacpi_status
uacpi_eval_uid (uacpi_namespace_node *node, uacpi_id_string **out_uid)
{
	uacpi_status ret;
	uacpi_object *obj;
	uacpi_id_string *id_string;
...
static uacpi_bool
matches_any (uacpi_id_string *id, const uacpi_char *const *ids)
{
	uacpi_size i;
...
void
uacpi_free_namespace_node_info (uacpi_namespace_node_info *info)
{
	if (info == UACPI_NULL)
		return;
...
uacpi_bool
uacpi_device_matches_pnp_id (uacpi_namespace_node *node,
...
uacpi_status
uacpi_find_devices_at (uacpi_namespace_node *parent,
...
uacpi_status
uacpi_find_devices (const uacpi_char *hid, uacpi_iteration_callback cb,
										void *user)
{
...
uacpi_status
uacpi_set_interrupt_model (uacpi_interrupt_model model)
{
	uacpi_status ret;
	uacpi_object *arg;
...
uacpi_status
uacpi_get_pci_routing_table (uacpi_namespace_node *parent,
...
void
uacpi_free_pci_routing_table (uacpi_pci_routing_table *table)
{
	if (table == UACPI_NULL)
		return;

	uacpi_free (table, sizeof (uacpi_pci_routing_table)
												 + table->num_entries
															 * sizeof (uacpi_pci_routing_table_entry));
}

void
uacpi_free_dynamic_string (const uacpi_char *str)
{
	if (str == UACPI_NULL)
		return;
