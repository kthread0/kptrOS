...
void
uacpi_context_set_log_level (uacpi_log_level lvl)
{
	if (lvl == 0)
...
void
uacpi_context_set_proactive_table_checksum (uacpi_bool setting)
{
	if (setting)
...
const uacpi_char *
uacpi_status_to_string (uacpi_status st)
{
	switch (st)
		{
...
void
uacpi_context_set_loop_timeout (uacpi_u32 seconds)
{
	if (seconds == 0)
...
void
uacpi_context_set_max_call_stack_depth (uacpi_u32 depth)
{
	if (depth == 0)
...
uacpi_u32
uacpi_context_get_loop_timeout (void)
{
...
static uacpi_status
enter_mode (enum hw_mode mode, uacpi_bool *did_change)
{
	uacpi_status ret;
...
uacpi_init_level
uacpi_get_current_init_level (void)
{
	return g_uacpi_rt_ctx.init_level;
}

uacpi_status
uacpi_initialize (uacpi_u64 flags)
{
	uacpi_status ret;
...
static void
trace_table_load_failure (struct acpi_sdt_hdr *tbl, uacpi_log_level lvl,
													uacpi_status ret)
{
...
static uacpi_bool
match_ssdt_or_psdt (struct uacpi_installed_table *tbl)
{
...
static uacpi_u64
elapsed_ms (uacpi_u64 begin_ns, uacpi_u64 end_ns)
{
	return (end_ns - begin_ns) / (1000ull * 1000ull);
}

static uacpi_bool
warn_on_bad_timesource (uacpi_u64 begin_ts, uacpi_u64 end_ts)
{
	...
	for (cur_index = 0;; cur_index = tbl.index + 1)
		{
			...
			ret = uacpi_table_load_with_cause (tbl.index,
...
static void
ini_eval (struct ns_init_context *ctx, uacpi_namespace_node *node)
{
	uacpi_status ret;
...
static uacpi_status
sta_eval (struct ns_init_context *ctx, uacpi_namespace_node *node,
					uacpi_u32 *value)
{
	uacpi_status ret;
...
uacpi_status
uacpi_namespace_initialize (void)
{
	...
	if (obj->type != UACPI_OBJECT_METHOD)
		{
			...
			ret = uacpi_object_assign (new_obj, obj,
...
uacpi_status
uacpi_eval_simple (uacpi_namespace_node *parent, const uacpi_char *path,
									 uacpi_object **ret)
{
...
uacpi_status
uacpi_execute (uacpi_namespace_node *parent, const uacpi_char *path,
...
uacpi_status
uacpi_execute_simple (uacpi_namespace_node *parent, const uacpi_char *path)
{
...
static void
trace_invalid_return_type (uacpi_namespace_node *parent,
...
uacpi_status
uacpi_eval_typed (uacpi_namespace_node *parent, const uacpi_char *path,
									const uacpi_object_array *args,
...
uacpi_status
uacpi_eval_simple_typed (uacpi_namespace_node *parent, const uacpi_char *path,
...
uacpi_status
uacpi_eval_integer (uacpi_namespace_node *parent, const uacpi_char *path,
...
uacpi_status
uacpi_eval_simple_integer (uacpi_namespace_node *parent,
													 const uacpi_char *path, uacpi_u64 *out_value)
{
	return uacpi_eval_integer (parent, path, UACPI_NULL, out_value);
}

uacpi_status
uacpi_eval_buffer_or_string (uacpi_namespace_node *parent,
														 const uacpi_char *path,
														 const uacpi_object_array *args,
														 uacpi_object **ret)
{
...
uacpi_status
uacpi_eval_simple_buffer_or_string (uacpi_namespace_node *parent,
...
uacpi_status
uacpi_eval_string (uacpi_namespace_node *parent, const uacpi_char *path,
...
uacpi_status
uacpi_eval_simple_string (uacpi_namespace_node *parent, const uacpi_char *path,
													uacpi_object **ret)
{
...
uacpi_status
uacpi_eval_buffer (uacpi_namespace_node *parent, const uacpi_char *path,
									 const uacpi_object_array *args, uacpi_object **ret)
{
...
uacpi_status
uacpi_eval_simple_buffer (uacpi_namespace_node *parent, const uacpi_char *path,
													uacpi_object **ret)
{
...
uacpi_status
uacpi_eval_package (uacpi_namespace_node *parent, const uacpi_char *path,
										const uacpi_object_array *args, uacpi_object **ret)
{
...
uacpi_status
uacpi_eval_simple_package (uacpi_namespace_node *parent,
													 const uacpi_char *path, uacpi_object **ret)
{
...
uacpi_status
uacpi_get_aml_bitness (uacpi_u8 *out_bitness)
{
	UACPI_ENSURE_INIT_LEVEL_AT_LEAST (UACPI_INIT_LEVEL_SUBSYSTEM_INITIALIZED);
