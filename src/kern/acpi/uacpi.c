...
#	endif
	...
	if (uacpi_likely(!g_uacpi_rt_ctx.bad_timesource)) {
		uacpi_info("namespace initialization done in %" UACPI_PRIu64
							 "ms: "
		...
		uacpi_info("namespace initialization done: %zu devices, %zu thermal zones\n", ctx.devices, ctx.thermal_zones);
	}
...
uacpi_status
								uacpi_eval(uacpi_namespace_node *parent,
	...
	if (path != UACPI_NULL) {
		ret = uacpi_namespace_node_resolve(
										parent, path, UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_NO, UACPI_PERMANENT_ONLY_YES, &node);
		if (uacpi_unlikely_error(ret))
...
uacpi_status uacpi_eval_simple_typed(
								uacpi_namespace_node *parent, const uacpi_char *path, uacpi_object_type_bits ret_mask, uacpi_object **ret) {
	return uacpi_eval_typed(parent, path, UACPI_NULL, ret_mask, ret);
}

uacpi_status uacpi_eval_integer(
								uacpi_namespace_node *parent, const uacpi_char *path, const uacpi_object_array *args, uacpi_u64 *out_value) {
...
uacpi_status uacpi_eval_buffer_or_string(
								uacpi_namespace_node *parent, const uacpi_char *path, const uacpi_object_array *args, uacpi_object **ret) {
