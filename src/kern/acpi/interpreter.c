...
static uacpi_status held_mutexes_array_remove_and_release(
								struct held_mutexes_array *arr, uacpi_mutex *mutex, enum force_release force) {
	uacpi_mutex *item;
	uacpi_size i;
...
static uacpi_status name_string_to_path(
								struct call_frame *frame, uacpi_size offset, uacpi_char **out_string, uacpi_size *out_size) {
...
static uacpi_status resolve_name_string(
								struct call_frame *frame, enum resolve_behavior behavior, struct uacpi_namespace_node **out_node) {
...
static uacpi_status handle_package(struct execution_context *ctx) {
	...
	for (i = 0; i < num_defined_elements; ++i) {
		...
		if (obj == UACPI_NULL) {
			...
			ret = name_string_to_path(ctx->cur_frame, item_array_at(&op_ctx->items, base_pkg_index)->immediate, &path, &length);
			if (uacpi_unlikely_error(ret))
				return ret;
...
static void debug_store_no_recurse(const uacpi_char *prefix, uacpi_object *src) {
	...
	case UACPI_OBJECT_MUTEX:
		uacpi_trace("%s Mutex @%p (%p => %p) sync level %d\n", prefix, src, src->mutex, src->mutex->handle, src->mutex->sync_level);
		break;
...
static uacpi_iteration_decision
								opregion_try_detach_from_parent(void *user, uacpi_namespace_node *node, uacpi_u32 node_depth) {
...
static void object_replace_child(uacpi_object *parent, uacpi_object *new_child) {
	if (parent->flags == UACPI_REFERENCE_KIND_NAMED &&
		...
		uacpi_namespace_do_for_each_child(
										...
										opregion_try_detach_from_parent,
										UACPI_NULL,
...
static uacpi_status do_binary_math(
								uacpi_object *arg0, uacpi_object *arg1, uacpi_object *tgt0, uacpi_object *tgt1, uacpi_aml_op op) {
...
static uacpi_status integer_to_string(uacpi_u64 integer, uacpi_buffer *str, uacpi_bool is_hex) {
	...
	repr_len = uacpi_snprintf(int_buf, sizeof(int_buf), is_hex ? "%" UACPI_PRIX64 : "%" UACPI_PRIu64, UACPI_FMT64(integer));
...
static uacpi_status handle_create_buffer_field(struct execution_context *ctx) {
	...
	if (uacpi_unlikely((field->bit_index + field->bit_length) > src_buf->size * 8)) {
		uacpi_error("invalid buffer field: bits [%zu..%zu], buffer size is %zu bytes\n",
								field->bit_index,
								field->bit_index + field->bit_length,
								src_buf->size);
		return UACPI_STATUS_AML_OUT_OF_BOUNDS_INDEX;
	}
...
static uacpi_status frame_setup_base_scope(
								struct call_frame *frame, uacpi_namespace_node *scope, uacpi_control_method *method) {
	struct code_block *block;
...
static uacpi_status
								typecheck_obj(const struct op_context *op_ctx,
		...
		case UACPI_PARSE_OP_EXISTING_NAMESTRING_OR_NULL_IF_LOAD: {
			...
			if (uacpi_unlikely_error(ret)) {
				...
				if (frame->method->named_objects_persist &&
					...
					abort_whileif = first_ctx->op->code == UACPI_AML_OP_WhileOp || first_ctx->op->code == UACPI_AML_OP_IfOp;
...
static void execution_context_release(struct execution_context *ctx) {
	...
	while (held_mutexes_array_size(&ctx->held_mutexes) != 0) {
		held_mutexes_array_remove_and_release(
										&ctx->held_mutexes, *held_mutexes_array_last(&ctx->held_mutexes), FORCE_RELEASE_YES);
	}
