...
static uacpi_status temp_namespace_node_array_push(
		struct temp_namespace_node_array* arr,
...
static uacpi_status handle_package(struct execution_context* ctx) {
	...
	if (op_ctx->op->code == UACPI_AML_OP_VarPackageOp) {
		...
		if (uacpi_unlikely(var_num_elements->integer > 0xE0000000)) {
			uacpi_error("package is too large (%" UACPI_PRIu64
									"), assuming "
...
static uacpi_status handle_create_op_region(struct execution_context* ctx) {
	...
	if (uacpi_unlikely(op_region->length == 0)) {
		...
		uacpi_error("invalid operation region %.4s bounds: offset=0x%" UACPI_PRIX64
...
static void prepare_table_load(void* ptr,
	...
	if (is_dynamic_table_load(cause)) {
		log_prefix = cause == UACPI_TABLE_LOAD_CAUSE_HOST ? "host-invoked load of"
																											: "dynamic load of";
...
static uacpi_status handle_load_table(struct execution_context* ctx) {
	...
	if (param_path->size > 1) {
		...
		ret = uacpi_namespace_node_resolve(root_node, param_path->text,
																			 UACPI_SHOULD_LOCK_NO,
																			 UACPI_MAY_SEARCH_ABOVE_PARENT_YES,
																			 UACPI_PERMANENT_ONLY_NO, &param_node);
		if (uacpi_unlikely_error(ret)) {
...
static void debug_store_no_recurse(const uacpi_char* prefix,
	...
	switch (src->type) {
		...
		case UACPI_OBJECT_PACKAGE:
			...
			break;
		case UACPI_OBJECT_BUFFER:
			uacpi_trace("%s Buffer @%p (%p) (%zu bytes)\n", prefix, src, src->buffer,
									src->buffer->size);
			break;
		case UACPI_OBJECT_OPERATION_REGION:
			uacpi_trace("%s OperationRegion (ASID %d) 0x%016" UACPI_PRIX64
									" -> 0x%016" UACPI_PRIX64 "\n",
									prefix, src->op_region->space,
									UACPI_FMT64(src->op_region->offset),
		...
		case UACPI_OBJECT_PROCESSOR:
			uacpi_trace("%s Processor[%d] 0x%08X (%d)\n", prefix, src->processor->id,
									src->processor->block_address, src->processor->block_length);
			break;
		case UACPI_OBJECT_BUFFER_INDEX:
		...
		case UACPI_OBJECT_METHOD:
			...
			break;
		default:
			uacpi_trace("%s %s @%p\n", prefix, uacpi_object_type_to_string(src->type),
									src);
	}
}
...
static uacpi_status do_binary_math(uacpi_object* arg0,
																	 uacpi_object* arg1,
																	 uacpi_object* tgt0,
																	 uacpi_object* tgt1,
																	 uacpi_aml_op op) {
...
static uacpi_status ensure_valid_idx(uacpi_object* obj,
																		 uacpi_size idx,
																		 uacpi_size src_size) {
...
static uacpi_status handle_index(struct execution_context* ctx) {
	...
	switch (src->type) {
		...
		case UACPI_OBJECT_PACKAGE: {
			...
			if (obj->type != UACPI_OBJECT_REFERENCE ||
					obj->flags != UACPI_REFERENCE_KIND_PKG_INDEX) {
				obj = uacpi_create_internal_reference(UACPI_REFERENCE_KIND_PKG_INDEX,
																							obj);
				if (uacpi_unlikely(obj == UACPI_NULL))
					return UACPI_STATUS_OUT_OF_MEMORY;
...
static uacpi_status integer_to_string(uacpi_u64 integer,
																			uacpi_buffer* str,
																			uacpi_bool is_hex) {
	int repr_len;
...
static uacpi_status buffer_to_string(uacpi_buffer* buf,
																		 uacpi_buffer* str,
																		 uacpi_bool is_hex) {
	int repr_len;
...
static uacpi_status handle_create_mutex_or_event(
	...
	switch (op_ctx->op->code) {
		case UACPI_AML_OP_AcquireOp: {
			...
			if (uacpi_unlikely(ctx->sync_level > obj->mutex->sync_level)) {
				uacpi_warn(
						"ignoring attempt to acquire mutex @%p with a lower "
						"sync level "
						"(%d < %d)\n",
		...
		case UACPI_AML_OP_ReleaseOp: {
			...
			ret = held_mutexes_array_remove_and_release(&ctx->held_mutexes,
																									obj->mutex, FORCE_RELEASE_NO);
			if (uacpi_likely_success(ret)) {
...
static uacpi_status handle_notify(struct execution_context* ctx) {
	...
	if (ret == UACPI_STATUS_NO_HANDLER) {
		...
		uacpi_warn("ignoring firmware Notify(%s, 0x%" UACPI_PRIX64
							 ") request, "
							 "no listeners\n",
...
static uacpi_status handle_field_read(struct execution_context* ctx) {
	...
	ret =
			uacpi_read_field_unit(src_obj->field_unit, dst, dst_size, &wtr_response);
	if (uacpi_unlikely_error(ret))
		return ret;
...
static uacpi_status handle_create_buffer_field(struct execution_context* ctx) {
	...
	if (uacpi_unlikely((field->bit_index + field->bit_length) >
		...
		uacpi_error(
				...
				field->bit_index, field->bit_index + field->bit_length, src_buf->size);
		return UACPI_STATUS_AML_OUT_OF_BOUNDS_INDEX;
	}
...
static inline void trace_op(const struct uacpi_op_spec* op,
	...
	uacpi_debug("%s OP '%s' (0x%04X)\n", op_trace_action_types[action], op->name,
							op->code);
}
...
static uacpi_status store_to_target(uacpi_object* dst,
																		uacpi_object* src,
																		uacpi_data_view* wtr_response) {
	uacpi_status ret;
...
static uacpi_status op_typecheck(const struct op_context* op_ctx,
	...
	if (!(props & ok_mask)) {
		EXEC_OP_ERR_2("invalid argument: '%s', expected a %s", cur_op_ctx->op->name,
									expected_type_str);
		return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
	}
	...
	for (;;) {
		...
		switch (op) {
			...
			case UACPI_PARSE_OP_LOAD_INLINE_IMM_AS_OBJECT: {
				...
				uacpi_memcpy_zerout(dst, op_decode_cursor(op_ctx), sizeof(uacpi_u64),
														src_width);
				op_ctx->pc += src_width;
				break;
			}
			...
			case UACPI_PARSE_OP_DISPATCH_METHOD_CALL: {
				...
				ret =
						prepare_method_call(ctx, node, method, METHOD_CALL_AML, UACPI_NULL);
				return ret;
			}
...
static void stack_unwind(struct execution_context* ctx) {
	...
	if (depth != 0) {
		...
		do {
			...
			trace_method_abort(code_block_array_at(&ctx->cur_frame->code_blocks, 0),
												 idx);
