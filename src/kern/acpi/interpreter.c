...
static uacpi_status
held_mutexes_array_push (struct held_mutexes_array *arr, uacpi_mutex *mutex)
{
	uacpi_mutex **slot;
...
static void
held_mutexes_array_remove_idx (struct held_mutexes_array *arr, uacpi_size i)
{
	uacpi_size size;
...
static void *
call_frame_cursor (struct call_frame *frame)
{
	return frame->method->code + frame->code_offset;
}

static uacpi_size
call_frame_code_bytes_left (struct call_frame *frame)
{
	return frame->method->size - frame->code_offset;
}

static uacpi_bool
call_frame_has_code (struct call_frame *frame)
{
...
static uacpi_status
parse_nameseg (uacpi_u8 *cursor, uacpi_object_name *out_name)
{
...
static uacpi_status
name_string_to_path (struct call_frame *frame, uacpi_size offset,
...
static uacpi_status
do_install_node_item (struct call_frame *frame, struct item *item)
{
	uacpi_status ret;
...
static uacpi_u8
peek_next_op (struct call_frame *frame, uacpi_aml_op *out_op)
{
	uacpi_aml_op op;
...
static uacpi_status
get_op (struct execution_context *ctx)
{
	uacpi_aml_op op;
	uacpi_u8 length;
...
static uacpi_status
handle_buffer (struct execution_context *ctx)
{
	struct package_length *pkg;
	uacpi_u8 *src;
...
static uacpi_status
handle_string (struct execution_context *ctx)
{
...
static uacpi_status
handle_package (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
get_object_storage (uacpi_object *obj, uacpi_data_view *out_buf,
...
static uacpi_u8 *
buffer_index_cursor (uacpi_buffer_index *buf_idx)
{
	uacpi_u8 *out_cursor;
...
static void
write_buffer_index (uacpi_buffer_index *buf_idx, uacpi_data_view *src_buf)
{
...
static uacpi_status
handle_arg_or_local (struct execution_context *ctx, uacpi_size idx,
...
static uacpi_status
handle_local (struct execution_context *ctx)
{
	uacpi_size idx;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_arg (struct execution_context *ctx)
{
	uacpi_size idx;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_named_object (struct execution_context *ctx)
{
...
static uacpi_status
handle_create_alias (struct execution_context *ctx)
{
...
static uacpi_status
handle_create_op_region (struct execution_context *ctx)
{
	uacpi_namespace_node *node;
	uacpi_object *obj;
...
static uacpi_status
table_id_error (const uacpi_char *opcode, const uacpi_char *arg,
								uacpi_buffer *str)
{
...
static void
report_table_id_find_error (const uacpi_char *opcode,
...
static uacpi_status
build_table_id (const uacpi_char *opcode,
								...
								uacpi_buffer *oem_table_id)
{
	...
	if (uacpi_unlikely (oem_table_id->size
											> (sizeof (out_id->oem_table_id) + 1)))
...
static uacpi_status
handle_create_data_region (struct execution_context *ctx)
{
	uacpi_status ret;
...
static uacpi_bool
is_dynamic_table_load (enum uacpi_table_load_cause cause)
{
	return cause != UACPI_TABLE_LOAD_CAUSE_INIT;
}

static void
prepare_table_load (void *ptr, enum uacpi_table_load_cause cause,
...
static uacpi_status
do_load_table (uacpi_namespace_node *parent, struct acpi_sdt_hdr *tbl,
...
static uacpi_status
handle_load_table (struct execution_context *ctx)
{
	uacpi_status ret;
	struct item_array *items = &ctx->cur_op_ctx->items;
...
static uacpi_status
handle_load (struct execution_context *ctx)
{
	uacpi_status ret;
	struct item_array *items = &ctx->cur_op_ctx->items;
	uacpi_table table;
...
uacpi_status
uacpi_execute_table (void *tbl, enum uacpi_table_load_cause cause)
{
	uacpi_status ret;
...
static uacpi_u32
get_field_length (struct item *item)
{
...
static uacpi_status
ensure_is_a_field_unit (uacpi_namespace_node *node,
...
static uacpi_status
ensure_is_an_op_region (uacpi_namespace_node *node,
...
static uacpi_status
handle_create_field (struct execution_context *ctx)
{
	uacpi_status ret;
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_namespace_node *node;
...
static void
truncate_number_if_needed (uacpi_object *obj)
{
...
static uacpi_status
method_get_ret_target (struct execution_context *ctx,
...
static uacpi_status
method_get_ret_object (struct execution_context *ctx, uacpi_object **out_obj)
{
	uacpi_status ret;
...
find_last_block (struct code_block_array *blocks, enum code_block_type type)
{
	uacpi_size i;
...
static void
update_scope (struct call_frame *frame)
{
	struct code_block *block;
...
static uacpi_status
begin_block_execution (struct execution_context *ctx)
{
		...
		case UACPI_AML_OP_WhileOp:
			...
			if (pkg->begin == cur_frame->prev_while_code_offset)
			...
			else
				{
					...
					block->expiration_point += g_uacpi_rt_ctx.loop_timeout_seconds
...
static void
frame_reset_post_end_block (struct execution_context *ctx,
														enum code_block_type type)
{
	struct call_frame *frame = ctx->cur_frame;
...
static void
debug_store_no_recurse (const uacpi_char *prefix, uacpi_object *src)
{
	switch (src->type)
		{
 ...
 */
static uacpi_object *
reference_unwind (uacpi_object *obj)
{
...
static void
object_replace_child (uacpi_object *parent, uacpi_object *new_child)
{
 ...
 */
static uacpi_status
copy_object_to_reference (uacpi_object *dst, uacpi_object *src)
{
	uacpi_status ret;
 ...
 */
static uacpi_status
store_to_reference (uacpi_object *dst, uacpi_object *src,
										uacpi_data_view *wtr_response)
{
...
static uacpi_status
handle_ref_or_deref_of (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
do_binary_math (uacpi_object *arg0, uacpi_object *arg1, uacpi_object *tgt0,
								uacpi_object *tgt1, uacpi_aml_op op)
{
...
static uacpi_status
handle_binary_math (struct execution_context *ctx)
{
...
static uacpi_status
handle_unary_math (struct execution_context *ctx)
{
...
static uacpi_status
ensure_valid_idx (uacpi_object *obj, uacpi_size idx, uacpi_size src_size)
{
...
static uacpi_status
handle_index (struct execution_context *ctx)
{
	uacpi_status ret;
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *src;
...
static uacpi_u64
object_to_integer (const uacpi_object *obj, uacpi_size max_buffer_bytes)
{
	uacpi_u64 dst;
...
static uacpi_status
integer_to_string (uacpi_u64 integer, uacpi_buffer *str, uacpi_bool is_hex)
{
	int repr_len;
...
static uacpi_status
buffer_to_string (uacpi_buffer *buf, uacpi_buffer *str, uacpi_bool is_hex)
{
	int repr_len;
...
static uacpi_status
do_make_empty_object (uacpi_buffer *buf, uacpi_bool is_string)
{
...
static uacpi_status
make_null_string (uacpi_buffer *buf)
{
	return do_make_empty_object (buf, UACPI_TRUE);
}

static uacpi_status
make_null_buffer (uacpi_buffer *buf)
{
	/*
...
static uacpi_status
handle_to (struct execution_context *ctx)
{
	uacpi_status ret = UACPI_STATUS_OK;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_to_string (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_mid (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *src, *dst;
	uacpi_data_view src_buf;
...
static uacpi_status
handle_concatenate (struct execution_context *ctx)
{
	uacpi_status ret = UACPI_STATUS_OK;
	struct op_context *op_ctx = ctx->cur_op_ctx;
		...
		case UACPI_OBJECT_BUFFER:
			{
				...
				uacpi_memcpy (dst_buf + arg0_buf->size, arg1_buf.bytes,
											arg1_buf.length);
				break;
			}
		case UACPI_OBJECT_STRING:
			{
				...
				switch (arg1->type)
					...
					case UACPI_OBJECT_INTEGER:
						{
							int size;
							size = uacpi_snprintf (int_buf, sizeof (int_buf),
																		 "%" UACPI_PRIx64,
...
static uacpi_status
handle_concatenate_res (struct execution_context *ctx)
{
	uacpi_status ret;
	struct op_context *op_ctx = ctx->cur_op_ctx;
	 ...
	 */
	dst_buf[dst_size - 2] = (ACPI_RESOURCE_END_TAG << ACPI_SMALL_ITEM_NAME_IDX)
...
static uacpi_status
handle_sizeof (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *src, *dst;
...
static uacpi_status
handle_object_type (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *src, *dst;
...
static uacpi_status
handle_timer (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *dst;
...
static uacpi_status
handle_stall_or_sleep (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_u64 time;
...
static uacpi_status
handle_bcd (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_u64 src, dst = 0;
	uacpi_size i;
...
static uacpi_status
handle_unload (struct execution_context *ctx)
{
	UACPI_UNUSED (ctx);
...
static uacpi_status
handle_logical_not (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *src, *dst;
...
static uacpi_bool
handle_logical_equality (uacpi_object *lhs, uacpi_object *rhs)
{
...
static uacpi_bool
handle_logical_less_or_greater (uacpi_aml_op op, uacpi_object *lhs,
																uacpi_object *rhs)
{
	if (lhs->type == UACPI_OBJECT_STRING || lhs->type == UACPI_OBJECT_BUFFER)
		{
...
static uacpi_status
handle_binary_logic (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_bool
match_one (enum match_op op, uacpi_u64 lhs, uacpi_u64 rhs)
{
	switch (op)
		{
		case MTR:
...
static uacpi_status
handle_match (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_package *pkg;
 ...
 */
static uacpi_status
parse_package_length (struct call_frame *frame, struct package_length *out_pkg)
{
	uacpi_u32 left, size;
 ...
 */
static void
init_method_flags (uacpi_control_method *method, uacpi_u8 flags_byte)
{
...
static uacpi_status
handle_create_method (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	...
	if (uacpi_unlikely (pkg->end < pkg->begin || pkg->end < method_begin_offset
...
static uacpi_status
handle_event_ctl (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *obj;
...
static uacpi_status
handle_mutex_ctl (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	uacpi_object *obj;
		...
		case UACPI_AML_OP_AcquireOp:
			{
				...
				if (uacpi_unlikely (ctx->sync_level > obj->mutex->sync_level))
					{
						uacpi_warn ("ignoring attempt to acquire mutex @%p with a lower "
												"sync level "
												"(%d < %d)\n",
...
static uacpi_status
handle_notify (struct execution_context *ctx)
{
	uacpi_status ret;
	struct op_context *op_ctx = ctx->cur_op_ctx;
	struct uacpi_namespace_node *node;
...
static uacpi_status
handle_firmware_request (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_create_named (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	struct uacpi_namespace_node *node;
	uacpi_object *src;
...
static uacpi_status
field_get_read_type (uacpi_object *obj, uacpi_object_type *out_type)
{
...
static uacpi_status
field_byte_size (uacpi_object *obj, uacpi_size *out_size)
{
...
static uacpi_status
handle_field_read (struct execution_context *ctx)
{
	uacpi_status ret;
	struct op_context *op_ctx = ctx->cur_op_ctx;
	struct uacpi_namespace_node *node;
...
static uacpi_status
handle_create_buffer_field (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
	struct uacpi_namespace_node *node;
...
static uacpi_status
handle_control_flow (struct execution_context *ctx)
{
	struct call_frame *frame = ctx->cur_frame;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
create_named_scope (struct op_context *op_ctx)
{
	uacpi_namespace_node *node;
	uacpi_object *obj;
		...
		case UACPI_AML_OP_PowerResOp:
			{
				...
				power_res->resource_order
						= item_array_at (&op_ctx->items, 3)->immediate;
				break;
			}
...
static uacpi_status
handle_code_block (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_return (struct execution_context *ctx)
{
	uacpi_status ret;
...
static void
refresh_ctx_pointers (struct execution_context *ctx)
{
	struct call_frame *frame = ctx->cur_frame;
...
static uacpi_bool
ctx_has_non_preempted_op (struct execution_context *ctx)
{
...
static inline void
trace_op (const struct uacpi_op_spec *op, enum op_trace_action_type action)
{
...
static inline void
trace_pop (uacpi_u8 pop)
{
	uacpi_debug ("    pOP: %s (0x%02X)\n", uacpi_parse_op_to_string (pop), pop);
}

static uacpi_status
frame_push_args (struct call_frame *frame, struct op_context *op_ctx)
{
	uacpi_size i;
...
static uacpi_status
frame_setup_base_scope (struct call_frame *frame, uacpi_namespace_node *scope,
												uacpi_control_method *method)
{
	struct code_block *block;
...
static uacpi_status
push_new_frame (struct execution_context *ctx, struct call_frame **out_frame)
{
...
static uacpi_bool
maybe_end_block (struct execution_context *ctx)
{
	struct code_block *block = ctx->cur_block;
	struct call_frame *cur_frame = ctx->cur_frame;
...
static uacpi_status
store_to_target (uacpi_object *dst, uacpi_object *src,
								 uacpi_data_view *wtr_response)
{
	uacpi_status ret;
...
static uacpi_status
handle_copy_object_or_store (struct execution_context *ctx)
{
	uacpi_object *src, *dst;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
handle_inc_dec (struct execution_context *ctx)
{
	uacpi_object *src, *dst;
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
enter_method (struct execution_context *ctx, struct call_frame *new_frame,
							uacpi_control_method *method)
{
	uacpi_status ret = UACPI_STATUS_OK;
...
static uacpi_status
push_op (struct execution_context *ctx)
{
	struct call_frame *frame = ctx->cur_frame;
	struct op_context *op_ctx;
...
static uacpi_bool
pop_item (struct op_context *op_ctx)
{
	struct item *item;
...
static void
pop_op (struct execution_context *ctx)
{
	struct call_frame *frame = ctx->cur_frame;
...
static void
call_frame_clear (struct call_frame *frame)
{
	uacpi_size i;
...
op_decode_cursor (const struct op_context *ctx)
{
...
static uacpi_u8
op_decode_byte (struct op_context *ctx)
{
	uacpi_u8 byte;
...
static uacpi_aml_op
op_decode_aml_op (struct op_context *op_ctx)
{
	uacpi_aml_op op = 0;
...
static uacpi_bool
op_wants_supername (enum uacpi_parse_op op)
{
	switch (op)
		{
...
static uacpi_bool
op_wants_term_arg_or_operand (enum uacpi_parse_op op)
{
	switch (op)
		{
...
static uacpi_bool
op_allows_unresolved (enum uacpi_parse_op op)
{
	switch (op)
		{
		case UACPI_PARSE_OP_SUPERNAME_OR_UNRESOLVED:
...
static uacpi_bool
op_allows_unresolved_if_load (enum uacpi_parse_op op)
{
	switch (op)
		{
...
static uacpi_status
op_typecheck (const struct op_context *op_ctx,
...
static uacpi_status
typecheck_obj (const struct op_context *op_ctx, const uacpi_object *obj,
...
static uacpi_status
typecheck_operand (const struct op_context *op_ctx, const uacpi_object *obj)
{
	return typecheck_obj (op_ctx, obj, UACPI_OBJECT_INTEGER, SPEC_OPERAND);
}

static uacpi_status
typecheck_string (const struct op_context *op_ctx, const uacpi_object *obj)
{
...
static void
emit_op_skip_warn (const struct op_context *op_ctx)
{
	EXEC_OP_WARN ("skipping due to previous errors");
}

static void
trace_named_object_lookup_or_creation_failure (struct call_frame *frame,
																							 uacpi_size offset,
																							 enum uacpi_parse_op op,
																							 uacpi_status ret,
...
static uacpi_status
uninstalled_op_handler (struct execution_context *ctx)
{
	struct op_context *op_ctx = ctx->cur_op_ctx;
...
static uacpi_status
prepare_method_call (struct execution_context *ctx, uacpi_namespace_node *node,
...
static void
apply_tracked_pkg (struct call_frame *frame, struct op_context *op_ctx)
{
	struct item *item;
...
static uacpi_status
exec_op (struct execution_context *ctx)
{
	uacpi_status ret = UACPI_STATUS_OK;
	struct call_frame *frame = ctx->cur_frame;
	struct op_context *op_ctx;
				...
				case UACPI_PARSE_OP_LOAD_TRUE_OBJECT:
					{
						...
						obj->integer
								= op == UACPI_PARSE_OP_LOAD_FALSE_OBJECT ? 0 : ones ();
						break;
					}
				...
				case UACPI_PARSE_OP_IF_LAST_NOT_NULL:
					{
						...
						if (op == UACPI_PARSE_OP_IF_LAST_NULL
							...
							{
								idx = op_decode_byte (op_ctx);
								is_null = item_array_at (&op_ctx->items, idx)->handle
													== UACPI_NULL;
							}
						...
						if (uacpi_likely_success (ret))
							{
								...
								if (op == UACPI_PARSE_OP_OBJECT_TRANSFER_TO_PREV)
									...
									{
										dst->obj
												= uacpi_create_object (UACPI_OBJECT_UNINITIALIZED);
										if (uacpi_unlikely (dst->obj == UACPI_NULL))
											{
												ret = UACPI_STATUS_OUT_OF_MEMORY;
												break;
										...
										ret = uacpi_object_assign (
												dst->obj, src, UACPI_ASSIGN_BEHAVIOR_DEEP_COPY);
									}
							}
						break;
					}
				...
				case UACPI_PARSE_OP_STORE_TO_TARGET_INDIRECT:
					{
						...
						if (op == UACPI_PARSE_OP_STORE_TO_TARGET_INDIRECT)
							{
								src = item_array_at (&op_ctx->items, op_decode_byte (op_ctx))
													->obj;
							}
						else
							{
				...
				case UACPI_PARSE_OP_DISPATCH_TABLE_LOAD:
					{
						...
						ret = prepare_method_call (ctx, node, method,
																			 METHOD_CALL_TABLE_LOAD, UACPI_NULL);
						return ret;
					}
...
static void
ctx_reload_post_ret (struct execution_context *ctx)
{
...
static void
trace_method_abort (struct code_block *block, uacpi_size depth)
{
...
static void
stack_unwind (struct execution_context *ctx)
{
	uacpi_size depth;
...
static void
execution_context_release (struct execution_context *ctx)
{
	if (ctx->ret)
...
uacpi_status
uacpi_execute_control_method (uacpi_namespace_node *scope,
															uacpi_control_method *method,
															const uacpi_object_array *args,
...
uacpi_status
uacpi_osi (uacpi_handle handle, uacpi_object *retval)
{
