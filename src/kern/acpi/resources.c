...
#	define OP(short_code, ...) { .code = UACPI_RESOURCE_CONVERT_OPCODE_##short_code, __VA_ARGS__ }

#	define END() OP(END)

#	define AML_O(short_aml_name, field) uacpi_offsetof(struct acpi_resource_##short_aml_name, field)

#	define AML_F(short_aml_name, field) .f1.aml_offset = AML_O(short_aml_name, field)

#	define NATIVE_O(short_name, field) uacpi_offsetof(uacpi_resource_##short_name, field)

#	define NATIVE_F(short_native_name, field) .f2.native_offset = NATIVE_O(short_native_name, field)
...
#	define CONVERT_TYPE_SPECIFIC_FLAGS(addr_type)                                       \
		OP(LOAD_8_STORE, AML_F(addr_type, common.type), NATIVE_F(addr_type, common.type)), \
										...
										OP(BIT_FIELD_2,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
											 NATIVE_F(addr_type, common.attribute.memory.caching),           \
											 IMM(1)),                                                        \
										OP(BIT_FIELD_2,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
											 NATIVE_F(addr_type, common.attribute.memory.range_type),        \
											 IMM(3)),                                                        \
										OP(BIT_FIELD_1,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
										...
										OP(BIT_FIELD_2,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
											 NATIVE_F(addr_type, common.attribute.io.range_type),            \
											 IMM(0)),                                                        \
										OP(BIT_FIELD_1,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
										...
										OP(BIT_FIELD_1,                                                    \
											 AML_F(addr_type, common.type_flags),                            \
											 NATIVE_F(addr_type, common.attribute.io.translation),           \
											 IMM(5)),                                                        \
										END(), /* Memory type that we don't know, just copy the byte */    \
										OP(FIELD_8,                                                        \
											 AML_F(addr_type, common.type_flags),                            \
											 NATIVE_F(addr_type, common.attribute.type_specific),            \
											 IMM(0xFF)),                                                     \
										END()
...
#	define CONVERT_GENERAL_ADDRESS_FLAGS(addr_type)                                                                    \
		OP(BIT_FIELD_1, AML_F(addr_type, common.flags), NATIVE_F(addr_type, common.direction), IMM(0)),                   \
										...
										OP(BIT_FIELD_1,                                                                                   \
											 ...
											 NATIVE_F(addr_type, common.fixed_min_address),                                                 \
											 IMM(2)),                                                                                       \
...
static const struct uacpi_resource_convert_instruction convert_gpio_connection[] = {
	...
	OP(BIT_FIELD_1, AML_F(gpio_connection, connection_flags), NATIVE_F(gpio_connection, intr.triggering), IMM(0)),
	OP(BIT_FIELD_2, AML_F(gpio_connection, connection_flags), NATIVE_F(gpio_connection, intr.polarity), IMM(1)),
	OP(BIT_FIELD_1, AML_F(gpio_connection, connection_flags), NATIVE_F(gpio_connection, intr.sharing), IMM(3)),
	OP(BIT_FIELD_1, AML_F(gpio_connection, connection_flags), NATIVE_F(gpio_connection, intr.wake_capability), IMM(4)),
	END(),
...
#	define DECODE_PIN_GROUP_RES_SOURCES(postfix)                                       \
		DECODE_SOURCE_INDEX(pin_group_##postfix),                                         \
										OP(RESOURCE_SOURCE_NO_INDEX,                                      \
											 NATIVE_F(pin_group_##postfix, source),                         \
...
static uacpi_iteration_decision
								find_end(void *opaque, uacpi_u8 *data, uacpi_u16 resource_size, const struct uacpi_resource_spec *spec) {
...
static uacpi_iteration_decision do_aml_resource_to_native(
								void *opaque, uacpi_u8 *data, uacpi_u16 aml_size, const struct uacpi_resource_spec *spec) {
	...
	for (;;) {
		...
		case UACPI_RESOURCE_CONVERT_OPCODE_RESOURCE_LABEL: {
			 ...
			 */
			if (insn->f3.arg2) {
				uacpi_memcpy_zerout(&max_offset, data + insn->f3.arg2, sizeof(max_offset), sizeof(uacpi_u16));
		...
		case UACPI_RESOURCE_CONVERT_OPCODE_PIN_TABLE: {
			...
			uacpi_memcpy(NATIVE_OFFSET(resource, insn->f3.arg2), &resource_end, sizeof(void *));
			break;
		}
		...
		case UACPI_RESOURCE_CONVERT_OPCODE_VENDOR_DATA: {
			...
			uacpi_memcpy(NATIVE_OFFSET(resource, insn->f3.arg2), &native_dst, sizeof(void *));
			break;
		}
...
static uacpi_iteration_decision accumulate_native_buffer_size(
								void *opaque, uacpi_u8 *data, uacpi_u16 resource_size, const struct uacpi_resource_spec *spec) {
	struct resource_conversion_ctx *ctx = opaque;
...
uacpi_status uacpi_for_each_device_resource(
								uacpi_namespace_node *device, const uacpi_char *method, uacpi_resource_iteration_callback cb, void *user) {
	uacpi_status ret;
	uacpi_resources *resources;
...
static uacpi_iteration_decision do_native_resource_to_aml(void *opaque, uacpi_resource *resource) {
	...
	for (;;) {
		...
		case UACPI_RESOURCE_CONVERT_OPCODE_VENDOR_DATA: {
			...
			uacpi_memcpy(&vendor_data, NATIVE_OFFSET(resource, insn->f3.arg2), sizeof(void *));
