...
/*
 * Map raw AML resource types to the internal enum, this also takes care of
 * type sanitization by returning UACPI_AML_RESOURCE_INVALID for any unknown
 * type.
 */
...
static const struct uacpi_resource_convert_instruction convert_vendor_type0[] =
		{
				...
				OP(FIELD_8,
					 AML_F(vendor_defined_type0, byte_data),
...
static const struct uacpi_resource_convert_instruction convert_vendor_type1[] =
		{
				OP(LOAD_AML_SIZE_32, NATIVE_F(vendor, length)),
				OP(FIELD_8,
					 AML_F(vendor_defined_type1, byte_data),
					 NATIVE_F(vendor, data)),
				END(),
};
...
static const struct uacpi_resource_convert_instruction convert_memory24[] = {
		OP(BIT_FIELD_1,
			 AML_F(memory24, information),
			 NATIVE_F(memory24, write_status),
			 IMM(0)),
...
static const struct uacpi_resource_convert_instruction convert_memory32[] = {
		OP(BIT_FIELD_1,
			 AML_F(memory32, information),
			 NATIVE_F(memory32, write_status),
			 IMM(0)),
...
static const struct uacpi_resource_convert_instruction
		convert_fixed_memory32[] = {
				OP(BIT_FIELD_1,
					 AML_F(fixed_memory32, information),
					 NATIVE_F(fixed_memory32, write_status),
					 IMM(0)),
				OP(FIELD_32,
					 AML_F(fixed_memory32, address),
					 NATIVE_F(fixed_memory32, address)),
				OP(FIELD_32,
					 AML_F(fixed_memory32, length),
...
static const struct uacpi_resource_convert_instruction
		convert_generic_register[] = {
				OP(FIELD_8,
					 ...
					 NATIVE_F(generic_register, address_space_id),
					 IMM(4)),
				OP(FIELD_64,
					 AML_F(generic_register, address),
...
static const struct uacpi_resource_convert_instruction convert_extended_irq[] =
		{
				OP(BIT_FIELD_1,
					 AML_F(extended_irq, flags),
					 NATIVE_F(extended_irq, direction),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(extended_irq, flags),
					 NATIVE_F(extended_irq, triggering),
					 IMM(1)),
				OP(BIT_FIELD_1,
					 AML_F(extended_irq, flags),
					 NATIVE_F(extended_irq, polarity),
					 IMM(2)),
				OP(BIT_FIELD_1,
					 AML_F(extended_irq, flags),
					 NATIVE_F(extended_irq, sharing),
					 IMM(3)),
				OP(BIT_FIELD_1,
					 AML_F(extended_irq, flags),
					 NATIVE_F(extended_irq, wake_capability),
					 IMM(4)),
				OP(LOAD_8_STORE,
					 AML_F(extended_irq, num_irqs),
					 NATIVE_F(extended_irq, num_irqs),
					 IMM(4)),
...
static const struct uacpi_resource_convert_instruction convert_clock_input[] = {
		...
		OP(BIT_FIELD_1,
			 AML_F(clock_input, flags),
			 NATIVE_F(clock_input, frequency),
			 IMM(0)),
		OP(BIT_FIELD_2,
			 AML_F(clock_input, flags),
			 NATIVE_F(clock_input, scale),
			 IMM(1)),
		...
		OP(FIELD_8,
			 AML_F(clock_input, source_index),
...
static const struct uacpi_resource_convert_instruction
		convert_gpio_connection[] = {
				...
				OP(FIELD_16,
					 ...
					 NATIVE_F(gpio_connection, drive_strength),
					 IMM(2)),
				DECODE_SOURCE_INDEX(gpio_connection),
				DECODE_RES_PIN_TBL_AND_VENDOR_DATA(gpio_connection,
																					 SOURCE_NO_INDEX,
																					 source_offset,
																					 source),
				OP(LOAD_8_STORE,
					 AML_F(gpio_connection, type),
				...
				OP(BIT_FIELD_1,
					 ...
					 NATIVE_F(gpio_connection, intr.triggering),
					 IMM(0)),
				OP(BIT_FIELD_2,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, intr.polarity),
					 IMM(1)),
				OP(BIT_FIELD_1,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, intr.sharing),
					 IMM(3)),
				OP(BIT_FIELD_1,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, intr.wake_capability),
					 IMM(4)),
				END(),
				OP(SKIP_IF_NOT_EQUALS, ARG0(UACPI_GPIO_CONNECTION_IO), IMM(3)),
				OP(BIT_FIELD_2,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, io.restriction),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, io.sharing),
					 IMM(3)),
				END(),
				OP(FIELD_16,
					 AML_F(gpio_connection, connection_flags),
					 NATIVE_F(gpio_connection, type_specific),
					 IMM(0xFF)),
				END(),
};
...
static const struct uacpi_resource_convert_instruction convert_pin_function[] =
		{
				...
				OP(BIT_FIELD_1,
					 AML_F(pin_function, flags),
					 NATIVE_F(pin_function, sharing),
					 IMM(0)),
				OP(FIELD_8,
				...
				DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_function,
																					 SOURCE_NO_INDEX,
																					 source_offset,
																					 source),
				END(),
...
static const struct uacpi_resource_convert_instruction
		convert_pin_configuration[] = {
				...
				OP(BIT_FIELD_1,
					 AML_F(pin_configuration, flags),
					 NATIVE_F(pin_configuration, sharing),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(pin_configuration, flags),
					 NATIVE_F(pin_configuration, direction),
					 IMM(1)),
				OP(FIELD_8,
					 AML_F(pin_configuration, type),
					 NATIVE_F(pin_configuration, type)),
				OP(FIELD_32,
					 AML_F(pin_configuration, value),
					 NATIVE_F(pin_configuration, value)),
				DECODE_SOURCE_INDEX(pin_configuration),
				DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_configuration,
																					 SOURCE_NO_INDEX,
																					 source_offset,
																					 source),
				END(),
...
static const struct uacpi_resource_convert_instruction convert_pin_group[] = {
		...
		OP(BIT_FIELD_1,
			 AML_F(pin_group, flags),
			 NATIVE_F(pin_group, direction),
			 IMM(0)),
		DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_group,
																			 LABEL,
																			 source_lable_offset,
																			 label),
		END(),
...
static const struct uacpi_resource_convert_instruction
		convert_pin_group_function[] = {
				...
				OP(BIT_FIELD_1,
					 AML_F(pin_group_function, flags),
					 NATIVE_F(pin_group_function, sharing),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(pin_group_function, flags),
					 NATIVE_F(pin_group_function, direction),
					 IMM(1)),
				OP(FIELD_16,
...
static const struct uacpi_resource_convert_instruction
		convert_pin_group_configuration[] = {
				...
				OP(BIT_FIELD_1,
					 AML_F(pin_group_configuration, flags),
					 NATIVE_F(pin_group_configuration, sharing),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(pin_group_configuration, flags),
					 NATIVE_F(pin_group_configuration, direction),
					 IMM(1)),
				OP(FIELD_8,
					 AML_F(pin_group_configuration, type),
					 NATIVE_F(pin_group_configuration, type)),
				OP(FIELD_32,
					 AML_F(pin_group_configuration, value),
...
static const struct uacpi_resource_convert_instruction
		convert_generic_serial_bus[] = {
				OP(FIELD_8,
					 AML_F(serial, revision_id),
					 NATIVE_F(serial_bus_common, revision_id)),
				OP(FIELD_8,
					 AML_F(serial, type_specific_revision_id),
					 NATIVE_F(serial_bus_common, type_revision_id)),
				OP(FIELD_8,
					 AML_F(serial, source_index),
					 NATIVE_F(serial_bus_common, source.index)),
				OP(FIELD_16,
					 AML_F(serial, type_data_length),
					 NATIVE_F(serial_bus_common, type_data_length)),
				OP(BIT_FIELD_1,
					 AML_F(serial, flags),
					 NATIVE_F(serial_bus_common, mode),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(serial, flags),
					 NATIVE_F(serial_bus_common, direction),
					 IMM(1)),
				OP(BIT_FIELD_1,
					 AML_F(serial, flags),
					 NATIVE_F(serial_bus_common, sharing),
					 IMM(2)),
				OP(SERIAL_TYPE_SPECIFIC,
					 AML_F(serial, type),
				...
				OP(BIT_FIELD_1,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(i2c_connection, addressing_mode),
					 IMM(0)),
				OP(FIELD_32,
					 AML_F(serial_i2c, connection_speed),
					 NATIVE_F(i2c_connection, connection_speed),
					 IMM(0xFF)),
				OP(FIELD_16,
				...
				OP(BIT_FIELD_1,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(spi_connection, wire_mode),
					 IMM(0)),
				OP(BIT_FIELD_1,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(spi_connection, device_polarity),
					 IMM(1)),
				OP(FIELD_32,
					 AML_F(serial_spi, connection_speed),
					 NATIVE_F(spi_connection, connection_speed),
					 IMM(0xFF)),
				OP(FIELD_8,
					 AML_F(serial_spi, data_bit_length),
					 NATIVE_F(spi_connection, data_bit_length),
					 IMM(5)),
				END(),
				OP(SKIP_IF_NOT_EQUALS, ARG0(ACPI_SERIAL_TYPE_UART), IMM(8)),
				OP(BIT_FIELD_2,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(uart_connection, flow_control),
					 IMM(0)),
				OP(BIT_FIELD_2,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(uart_connection, stop_bits),
					 IMM(2)),
				OP(BIT_FIELD_3,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(uart_connection, data_bits),
					 IMM(4)),
				OP(BIT_FIELD_1,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(uart_connection, endianness),
					 IMM(7)),
				OP(FIELD_32,
					 AML_F(serial_uart, baud_rate),
					 NATIVE_F(uart_connection, baud_rate),
					 IMM(0xFF)),
				OP(FIELD_16,
					 AML_F(serial_uart, rx_fifo),
					 NATIVE_F(uart_connection, rx_fifo),
					 IMM(2)),
				OP(FIELD_8,
					 AML_F(serial_uart, parity),
					 NATIVE_F(uart_connection, parity),
					 IMM(2)),
				END(),
				OP(SKIP_IF_NOT_EQUALS, ARG0(ACPI_SERIAL_TYPE_CSI2), IMM(3)),
				OP(BIT_FIELD_2,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(csi2_connection, phy_type),
					 IMM(0)),
				OP(BIT_FIELD_6,
					 AML_F(serial, type_specific_flags),
					 NATIVE_F(csi2_connection, local_port),
					 IMM(2)),
				END(),
...
const struct uacpi_resource_spec aml_resources[UACPI_AML_RESOURCE_MAX + 1] = {
		...
		DEFINE_SMALL_AML_RESOURCE_NO_NATIVE_REPR(
				...
				.size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED, ),
		DEFINE_SMALL_AML_RESOURCE(UACPI_AML_RESOURCE_IO,
															...
															.to_aml = convert_io, ),
		DEFINE_SMALL_AML_RESOURCE(UACPI_AML_RESOURCE_FIXED_IO,
		...
		DEFINE_SMALL_AML_RESOURCE(UACPI_AML_RESOURCE_FIXED_DMA,
		...
		DEFINE_SMALL_AML_RESOURCE_NO_NATIVE_REPR(
				...
				.size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED, ),
		DEFINE_LARGE_AML_RESOURCE(UACPI_AML_RESOURCE_MEMORY24,
		...
		DEFINE_LARGE_AML_RESOURCE(UACPI_AML_RESOURCE_MEMORY32,
		...
		DEFINE_LARGE_AML_RESOURCE(UACPI_AML_RESOURCE_FIXED_MEMORY32,
	...
	for (;;) {
		...
		switch (insn->code) {
			...
			case UACPI_RESOURCE_CONVERT_OPCODE_RESOURCE_LABEL: {
				...
				/*
				 * Check if the string is bounded by anything at the top. If not,
				 * we just assume it ends at the end of the resource.
				 */
				if (insn->f3.arg2) {
...
uacpi_status uacpi_native_resources_from_aml(uacpi_data_view aml_buffer,
	...
	ret =
			uacpi_for_each_aml_resource(aml_buffer, do_aml_resource_to_native, &ctx);
	if (uacpi_unlikely_error(ret)) {
			...
			case UACPI_RESOURCE_CONVERT_OPCODE_SERIAL_TYPE_SPECIFIC: {
				...
				*dst = serial_type;
				...
				if (uacpi_unlikely(resource->type !=
					...
					uacpi_error(
							"native serial resource type %d doesn't match "
							"expected %d\n",
							resource->type, aml_serial_to_native_type(serial_type));
					ctx->st = UACPI_STATUS_INVALID_ARGUMENT;
					return UACPI_ITERATION_DECISION_BREAK;
