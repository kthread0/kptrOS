...
static uacpi_size
aml_size_with_header (const struct uacpi_resource_spec *spec)
{
...
static uacpi_size
size_for_aml_irq (const struct uacpi_resource_spec *spec,
...
static uacpi_size
extra_size_for_native_vendor (const struct uacpi_resource_spec *spec,
															void *data, uacpi_size size)
{
	UACPI_UNUSED (spec);
	UACPI_UNUSED (data);
...
static uacpi_size
size_for_aml_vendor (const struct uacpi_resource_spec *spec,
										 uacpi_resource *resource)
{
...
static uacpi_size
extra_size_for_resource_source (uacpi_size base_size, uacpi_size reported_size)
{
...
static uacpi_size
size_for_aml_resource_source (uacpi_resource_source *source,
...
static uacpi_size
extra_size_for_extended_irq (const struct uacpi_resource_spec *spec,
														 void *data, uacpi_size size)
{
...
static uacpi_size
size_for_aml_pin_group (const struct uacpi_resource_spec *spec,
												uacpi_resource *resource)
{
...
static const struct uacpi_resource_convert_instruction convert_irq_to_native[]
		= {
				...
				OP (BIT_FIELD_1, AML_F (irq, flags), NATIVE_F (irq, triggering),
						IMM (0)),
				OP (BIT_FIELD_1, AML_F (irq, flags), NATIVE_F (irq, polarity),
						IMM (3)),
				...
				OP (SET_TO_IMM, NATIVE_F (irq, triggering),
						IMM (UACPI_TRIGGERING_EDGE)),
				END (),
			};
...
static const struct uacpi_resource_convert_instruction convert_memory24[] = {
	...
	OP (FIELD_16, AML_F (memory24, minimum), NATIVE_F (memory24, minimum),
			IMM (4)),
	END (),
};
...
static const struct uacpi_resource_convert_instruction convert_memory32[] = {
	...
	OP (FIELD_32, AML_F (memory32, minimum), NATIVE_F (memory32, minimum),
			IMM (4)),
	END (),
};
...
#define CONVERT_TYPE_SPECIFIC_FLAGS(addr_type)                                \
	OP (LOAD_8_STORE, AML_F (addr_type, common.type),                           \
			...
			OP (BIT_FIELD_1, AML_F (addr_type, common.type_flags),                  \
					NATIVE_F (addr_type, common.attribute.memory.write_status),         \
					IMM (0)),                                                           \
			...
			OP (BIT_FIELD_1, AML_F (addr_type, common.type_flags),                  \
					NATIVE_F (addr_type, common.attribute.memory.translation),          \
					IMM (5)),                                                           \
			END (), OP (SKIP_IF_NOT_EQUALS, ARG0 (UACPI_RANGE_IO), IMM (4)),        \
			OP (BIT_FIELD_2, AML_F (addr_type, common.type_flags),                  \
					NATIVE_F (addr_type, common.attribute.io.range_type), IMM (0)),     \
			OP (BIT_FIELD_1, AML_F (addr_type, common.type_flags),                  \
					NATIVE_F (addr_type, common.attribute.io.translation_type),         \
					IMM (4)),                                                           \
			OP (BIT_FIELD_1, AML_F (addr_type, common.type_flags),                  \
...
static const struct uacpi_resource_convert_instruction convert_extended_irq[]
		= {
				...
				OP (FIELD_8, AML_F (extended_irq, irqs),
						NATIVE_F (extended_irq, irqs)),
				END (),
			};
...
static const struct uacpi_resource_convert_instruction convert_clock_input[]
		= {
				...
				OP (BIT_FIELD_1, AML_F (clock_input, flags),
						NATIVE_F (clock_input, frequency), IMM (0)),
				OP (BIT_FIELD_2, AML_F (clock_input, flags),
						NATIVE_F (clock_input, scale), IMM (1)),
				OP (FIELD_16, AML_F (clock_input, divisor),
						NATIVE_F (clock_input, divisor)),
...
static const struct uacpi_resource_convert_instruction
		...
		= {
				...
				OP (SKIP_IF_NOT_EQUALS, ARG0 (UACPI_GPIO_CONNECTION_INTERRUPT),
						IMM (5)),
...
#define DEFINE_SMALL_AML_RESOURCE(aml_type_enum, native_type_enum,            \
	...
	[aml_type_enum]                                                             \
			= { .type = aml_type_enum,                                              \
					...
					.native_size                                                        \
					= sizeof (native_struct) + NATIVE_RESOURCE_HEADER_SIZE,             \
					__VA_ARGS__ }
...
const struct uacpi_resource_spec aml_resources[UACPI_AML_RESOURCE_MAX + 1] = {
	...
	DEFINE_LARGE_AML_RESOURCE (
			...
			.size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
			.to_native = convert_fixed_memory32, .to_aml = convert_fixed_memory32, ),
	DEFINE_LARGE_AML_RESOURCE (
...
static enum uacpi_aml_resource
get_aml_resource_type (uacpi_u8 raw_byte)
{
...
static uacpi_status
get_aml_resource_size (uacpi_u8 *data, uacpi_size bytes_left,
											 uacpi_u16 *out_size)
{
	uacpi_u16 size;
...
static uacpi_status
validate_aml_serial_type (uacpi_u8 type)
{
...
uacpi_status
uacpi_find_aml_resource_end_tag (uacpi_data_view buffer,
...
static uacpi_resource_type
aml_serial_to_native_type (uacpi_u8 type)
{
				...
				case UACPI_RESOURCE_CONVERT_OPCODE_BIT_FIELD_6:
					{
						...
						mask
								= (insn->code - UACPI_RESOURCE_CONVERT_OPCODE_BIT_FIELD_1) + 1;
						mask = (1 << mask) - 1;
...
static uacpi_status
eval_resource_helper (uacpi_namespace_node *node, const uacpi_char *method,
...
uacpi_status
uacpi_native_resources_from_aml (uacpi_data_view aml_buffer,
...
uacpi_status
uacpi_get_resource_from_buffer (uacpi_data_view aml_buffer,
...
void
uacpi_free_resources (uacpi_resources *resources)
{
...
void
uacpi_free_resource (uacpi_resource *resource)
{
...
uacpi_status
uacpi_get_current_resources (uacpi_namespace_node *device,
														 uacpi_resources **out_resources)
{
...
uacpi_status
uacpi_get_possible_resources (uacpi_namespace_node *device,
															uacpi_resources **out_resources)
{
...
uacpi_status
uacpi_get_device_resources (uacpi_namespace_node *device,
														const uacpi_char *method,
														uacpi_resources **out_resources)
{
...
uacpi_status
uacpi_for_each_resource (uacpi_resources *resources,
...
uacpi_status
uacpi_for_each_device_resource (uacpi_namespace_node *device,
																const uacpi_char *method,
				...
				case UACPI_RESOURCE_CONVERT_OPCODE_VENDOR_DATA:
					{
						...
						uacpi_memcpy (&vendor_data,
													NATIVE_OFFSET (resource, insn->f3.arg2),
													sizeof (void *));
...
static uacpi_status
native_resources_to_aml (uacpi_resources *native_resources, void *aml_buffer)
{
	uacpi_status ret;
	struct resource_conversion_ctx ctx = { 0 };
...
uacpi_status
uacpi_native_resources_to_aml (uacpi_resources *resources,
...
uacpi_status
uacpi_set_resources (uacpi_namespace_node *device, uacpi_resources *resources)
{
	uacpi_status ret;
