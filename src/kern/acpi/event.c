...
static uacpi_status create_gpe_block(
	...
	uacpi_trace("initialized GPE block %.4s[%d->%d], %d AML handlers (IRQ %d)\n",
							device_node->name.text,
							base_idx,
							base_idx + block->num_events,
							match_ctx.matched_count,
							irq);
	return UACPI_STATUS_OK;
...
static uacpi_status sanitize_device_and_find_gpe(
								uacpi_namespace_node **gpe_device, uacpi_u16 idx, struct gp_event **out_event) {
...
static uacpi_status do_install_gpe_handler(
	...
	if ((event->handler_type == GPE_HANDLER_TYPE_AML_HANDLER || event->handler_type == GPE_HANDLER_TYPE_IMPLICIT_NOTIFY) &&
			event->num_users != 0) {
...
static uacpi_status initialize_gpes(void) {
	...
	if (fadt->x_gpe0_blk.address && fadt->gpe0_blk_len) {
		...
		ret = create_gpe_block(
										gpe_node, fadt->sci_int, 0, fadt->x_gpe0_blk.address, fadt->x_gpe0_blk.address_space_id, gpe0_regs);
		if (uacpi_unlikely_error(ret)) {
	...
	if (fadt->x_gpe1_blk.address && fadt->gpe1_blk_len) {
		...
		if (uacpi_unlikely((gpe0_regs * EVENTS_PER_GPE_REGISTER) > fadt->gpe1_base)) {
			uacpi_error("FADT GPE block 1 [%d->%d] collides with GPE block 0 "
									"[%d->%d], ignoring\n",
									0,
									gpe0_regs * EVENTS_PER_GPE_REGISTER,
									fadt->gpe1_base,
...
uacpi_status uacpi_initialize_events(void) {
	...
	ret = uacpi_kernel_install_interrupt_handler(
									g_uacpi_rt_ctx.fadt.sci_int, handle_sci, g_gpe_interrupt_head, &g_uacpi_rt_ctx.sci_handle);
	if (uacpi_unlikely_error(ret)) {
