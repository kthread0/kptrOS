...
static uacpi_status
set_event (uacpi_u8 event, uacpi_u8 value)
{
	uacpi_status ret;
	uacpi_u64 raw_value;
...
dispatch_fixed_event (const struct fixed_event *ev, uacpi_fixed_event event)
{
	uacpi_status ret;
...
static uacpi_u8
gpe_get_mask (struct gp_event *event)
{
...
static uacpi_status
set_gpe_state (struct gp_event *event, enum gpe_state state)
{
	uacpi_status ret;
...
static uacpi_status
clear_gpe (struct gp_event *event)
{
	struct gpe_register *reg = event->reg;
...
static uacpi_status
restore_gpe (struct gp_event *event)
{
	uacpi_status ret;
...
static void
async_restore_gpe (uacpi_handle opaque)
{
	uacpi_status ret;
...
static void
async_run_gpe_handler (uacpi_handle opaque)
{
	uacpi_status ret;
	struct gp_event *event = opaque;
		...
		case GPE_HANDLER_TYPE_IMPLICIT_NOTIFY:
			{
				...
				while (handler)
					{
						/*
						 * 2 - Device Wake. Used to notify OSPM that the device has
						 * signaled its wake event, and that OSPM needs to notify OSPM
						 * native device driver for the device.
						 */
...
static uacpi_interrupt_ret
dispatch_gpe (uacpi_namespace_node *device_node, struct gp_event *event)
{
	uacpi_status ret;
	uacpi_interrupt_ret int_ret = UACPI_INTERRUPT_NOT_HANDLED;
...
static uacpi_interrupt_ret
detect_gpes (struct gpe_block *block)
{
	uacpi_status ret;
	uacpi_interrupt_ret int_ret = UACPI_INTERRUPT_NOT_HANDLED;
	struct gpe_register *reg;
...
static uacpi_status
maybe_dispatch_gpe (uacpi_namespace_node *gpe_device, struct gp_event *event)
{
	uacpi_status ret;
	struct gpe_register *reg = event->reg;
	uacpi_u64 status;
...
static uacpi_interrupt_ret
handle_gpes (uacpi_handle opaque)
{
...
static void
gpe_release_implicit_notify_handlers (struct gp_event *event)
{
...
static uacpi_status
gpe_block_apply_action (struct gpe_block *block, enum gpe_block_action action)
{
	uacpi_status ret;
	uacpi_size i;
	uacpi_u8 value;
...
static void
gpe_block_mask_safe (struct gpe_block *block)
{
	uacpi_size i;
	struct gpe_register *reg;
...
static void
uninstall_gpe_block (struct gpe_block *block)
{
...
static struct gp_event *
gpe_from_block (struct gpe_block *block, uacpi_u16 idx)
{
	uacpi_u16 offset;
...
do_match_gpe_methods (uacpi_handle opaque, uacpi_namespace_node *node,
											uacpi_u32 depth)
{
	uacpi_status ret;
...
void
uacpi_events_match_post_dynamic_table_load (void)
{
...
static uacpi_status
create_gpe_block (uacpi_namespace_node *device_node, uacpi_u32 irq,
									uacpi_u16 base_idx, uacpi_u64 address,
									uacpi_u8 address_space_id, uacpi_u16 num_registers)
{
	...
	block->events = uacpi_kernel_alloc_zeroed (block->num_events
																						 * sizeof (*block->events));
	if (uacpi_unlikely (block->events == UACPI_NULL))
		goto error_out;

	for (reg = block->registers, event = block->events, i = 0; i < num_registers;
			 ++i, ++reg)
		{
...
static void
for_each_gpe_block (gpe_block_iteration_callback cb, uacpi_handle handle)
{
...
static uacpi_iteration_decision
do_find_gpe (struct gpe_block *block, uacpi_handle opaque)
{
...
static struct gp_event *
get_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
...
static void
gp_event_toggle_masks (struct gp_event *event, uacpi_bool set_on)
{
	uacpi_u8 this_mask;
	struct gpe_register *reg = event->reg;
...
static uacpi_status
gpe_remove_user (struct gp_event *event)
{
...
static uacpi_status
gpe_add_user (struct gp_event *event, enum event_clear_if_first clear_if_first)
{
	uacpi_status ret = UACPI_STATUS_OK;
...
static uacpi_bool
gpe_needs_polling (struct gp_event *event)
{
	return event->num_users && event->triggering == UACPI_GPE_TRIGGERING_EDGE;
}

static uacpi_status
gpe_mask_unmask (struct gp_event *event, uacpi_bool should_mask)
{
	struct gpe_register *reg;
	uacpi_u8 mask;
 ...
 */
static uacpi_bool
gpe_mask_safe (struct gp_event *event)
{
	// No need to flush or do anything if it's not currently enabled
...
static uacpi_iteration_decision
do_initialize_gpe_block (struct gpe_block *block, uacpi_handle opaque)
{
	uacpi_status ret;
...
uacpi_status
uacpi_finalize_gpe_initialization (void)
{
	uacpi_status ret;
...
static uacpi_status
do_install_gpe_handler (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
												uacpi_gpe_triggering triggering,
												enum gpe_handler_type type, uacpi_gpe_handler handler,
												uacpi_handle ctx)
{
	uacpi_status ret;
	struct gp_event *event;
...
uacpi_status
uacpi_install_gpe_handler (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
													 uacpi_gpe_triggering triggering,
...
uacpi_status
uacpi_install_gpe_handler_raw (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
															 uacpi_gpe_triggering triggering,
															 uacpi_gpe_handler handler, uacpi_handle ctx)
{
	return do_install_gpe_handler (gpe_device, idx, triggering,
...
uacpi_status
uacpi_uninstall_gpe_handler (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
...
uacpi_status
uacpi_enable_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	uacpi_status ret;
	struct gp_event *event;
...
uacpi_status
uacpi_disable_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	uacpi_status ret;
	struct gp_event *event;
...
uacpi_status
uacpi_clear_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	uacpi_status ret;
	struct gp_event *event;
...
static uacpi_status
gpe_suspend_resume (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
...
uacpi_status
uacpi_suspend_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	return gpe_suspend_resume (gpe_device, idx, GPE_STATE_DISABLED);
}

uacpi_status
uacpi_resume_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	return gpe_suspend_resume (gpe_device, idx, GPE_STATE_ENABLED);
}

uacpi_status
uacpi_finish_handling_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	uacpi_status ret;
	struct gp_event *event;
...
static uacpi_status
gpe_get_mask_unmask (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
...
uacpi_status
uacpi_mask_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	return gpe_get_mask_unmask (gpe_device, idx, UACPI_TRUE);
}

uacpi_status
uacpi_unmask_gpe (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	return gpe_get_mask_unmask (gpe_device, idx, UACPI_FALSE);
}

uacpi_status
uacpi_setup_gpe_for_wake (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
...
uacpi_status
uacpi_enable_gpe_for_wake (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
	return gpe_enable_disable_for_wake (gpe_device, idx, UACPI_TRUE);
}

uacpi_status
uacpi_disable_gpe_for_wake (uacpi_namespace_node *gpe_device, uacpi_u16 idx)
{
...
static uacpi_iteration_decision
do_for_all_gpes (struct gpe_block *block, uacpi_handle opaque)
{
...
static uacpi_status
for_all_gpes_locked (struct do_for_all_gpes_ctx *ctx)
{
	uacpi_status ret;
...
uacpi_status
uacpi_enable_all_runtime_gpes (void)
{
	struct do_for_all_gpes_ctx ctx = {
...
uacpi_status
uacpi_enable_all_wake_gpes (void)
{
	struct do_for_all_gpes_ctx ctx = {
...
uacpi_status
uacpi_install_gpe_block (uacpi_namespace_node *gpe_device, uacpi_u64 address,
...
uacpi_status
uacpi_uninstall_gpe_block (uacpi_namespace_node *gpe_device)
{
	uacpi_status ret;
	uacpi_bool is_dev;
...
uacpi_status
uacpi_initialize_events_early (void)
{
	uacpi_status ret;
...
void
uacpi_deinitialize_events (void)
{
...
uacpi_status
uacpi_install_fixed_event_handler (uacpi_fixed_event event,
																	 uacpi_interrupt_handler handler,
																	 uacpi_handle user)
{
	uacpi_status ret;
...
uacpi_status
uacpi_uninstall_fixed_event_handler (uacpi_fixed_event event)
{
	uacpi_status ret;
	struct fixed_event_handler *ev;
...
uacpi_status
uacpi_fixed_event_info (uacpi_fixed_event event, uacpi_event_info *out_info)
{
	uacpi_status ret;
...
uacpi_status
uacpi_gpe_info (uacpi_namespace_node *gpe_device, uacpi_u16 idx,
