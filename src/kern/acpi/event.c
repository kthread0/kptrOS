...
static void async_run_gpe_handler(uacpi_handle opaque) {
	...
	switch (event->handler_type) {
		case GPE_HANDLER_TYPE_AML_HANDLER: {
			...
			ret = uacpi_execute_control_method(event->aml_handler, method_obj->method,
...
static uacpi_interrupt_ret dispatch_gpe(uacpi_namespace_node* device_node,
	...
	if (event->handler_type == GPE_HANDLER_TYPE_NATIVE_HANDLER_RAW) {
		return event->native_handler->cb(event->native_handler->ctx, device_node,
																		 event->idx);
	}
...
static uacpi_status create_gpe_block(uacpi_namespace_node* device_node,
	...
	uacpi_namespace_do_for_each_child(device_node, do_match_gpe_methods,
																		UACPI_NULL, UACPI_OBJECT_METHOD_BIT,
																		UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_YES,
																		UACPI_PERMANENT_ONLY_YES, &match_ctx);
...
uacpi_status uacpi_install_gpe_handler(uacpi_namespace_node* gpe_device,
	...
	return do_install_gpe_handler(gpe_device, idx, triggering,
																GPE_HANDLER_TYPE_NATIVE_HANDLER, handler, ctx);
}
...
static uacpi_status initialize_gpes(void) {
	...
	if (fadt->x_gpe0_blk.address && fadt->gpe0_blk_len) {
		...
		ret = create_gpe_block(gpe_node, fadt->sci_int, 0, fadt->x_gpe0_blk.address,
...
uacpi_status uacpi_initialize_events(void) {
	...
	ret = uacpi_kernel_install_interrupt_handler(g_uacpi_rt_ctx.fadt.sci_int,
																							 handle_sci, g_gpe_interrupt_head,
