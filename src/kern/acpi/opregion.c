...
uacpi_status
uacpi_initialize_opregion (void)
{
	return uacpi_recursive_lock_init (&g_opregion_lock);
}

void
uacpi_deinitialize_opregion (void)
{
	uacpi_recursive_lock_deinit (&g_opregion_lock);
}

void
uacpi_trace_region_error (uacpi_namespace_node *node, uacpi_char *message,
													uacpi_status ret)
{
...
static void
trace_region_io (uacpi_field_unit *field, uacpi_address_space space,
...
static uacpi_bool
space_needs_reg (enum uacpi_address_space space)
{
...
static uacpi_status
region_run_reg (uacpi_namespace_node *node, uacpi_u8 connection_code)
{
	uacpi_status ret;
...
uacpi_status
uacpi_opregion_attach (uacpi_namespace_node *node)
{
	uacpi_object *obj;
...
static void
region_install_handler (uacpi_namespace_node *node,
												uacpi_address_space_handler *handler)
{
	uacpi_operation_region *region;
...
static void
region_uninstall_handler (uacpi_namespace_node *node, enum unreg unreg)
{
	uacpi_status ret;
	uacpi_object *obj;
	uacpi_address_space_handler *handler;
...
void
uacpi_opregion_uninstall_handler (uacpi_namespace_node *node)
{
...
do_install_or_uninstall_handler (uacpi_handle opaque,
																 uacpi_namespace_node *node, uacpi_u32 depth)
{
...
uacpi_status
uacpi_reg_all_opregions (uacpi_namespace_node *device_node,
												 enum uacpi_address_space space)
{
	uacpi_status ret;
...
uacpi_status
uacpi_install_address_space_handler (uacpi_namespace_node *device_node,
																		 enum uacpi_address_space space,
...
uacpi_status
uacpi_initialize_opregion_node (uacpi_namespace_node *node)
{
	uacpi_status ret;
...
uacpi_bool
uacpi_is_buffer_access_address_space (uacpi_address_space space)
{
	switch (space)
		{
		case UACPI_ADDRESS_SPACE_SMBUS:
...
static uacpi_bool
space_needs_bounds_checking (uacpi_address_space space)
{
...
uacpi_status
uacpi_dispatch_opregion_io (uacpi_field_unit *field, uacpi_u32 offset,
														uacpi_region_op op,
														union uacpi_opregion_io_data data)
{
	uacpi_status ret;
