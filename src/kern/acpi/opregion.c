...
static void
								trace_region_io(uacpi_field_unit *field,
...
static uacpi_iteration_decision
								do_install_or_uninstall_handler(uacpi_handle opaque, uacpi_namespace_node *node, uacpi_u32 depth) {
...
static uacpi_status reg_or_unreg_all_opregions(
								uacpi_namespace_node *device_node, enum uacpi_address_space space, uacpi_u8 connection_code) {
	uacpi_address_space_handlers *handlers;
...
uacpi_status uacpi_dispatch_opregion_io(
	...
	if (uacpi_unlikely(is_oob)) {
		...
		uacpi_error("out-of-bounds access to opregion %s[0x%" UACPI_PRIX64
								...
								path,
								UACPI_FMT64(region->offset),
								UACPI_FMT64(region->offset + region->length),
								UACPI_FMT64(abs_offset),
								offset,
								field->access_width_bytes);
		uacpi_free_dynamic_string(path);
