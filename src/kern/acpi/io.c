...
static void cut_misaligned_tail(uacpi_u8* data,
																uacpi_size offset,
...
static uacpi_status access_field_unit(uacpi_field_unit* field,
																			uacpi_u32 offset,
																			uacpi_region_op op,
	...
	switch (region->space) {
		...
		case UACPI_ADDRESS_SPACE_PRM:
			if (uacpi_unlikely(op == UACPI_REGION_OP_READ)) {
				...
				uacpi_trace_region_error(field->region,
																 "attempted to read from a write-only", ret);
				goto out_handled;
			}
			UACPI_FALLTHROUGH;
		case UACPI_ADDRESS_SPACE_FFIXEDHW:
		...
		if (dst_span.index != 0 || dst_span.length < width_access_bits) {
			switch (field->update_rule) {
				case UACPI_UPDATE_RULE_PRESERVE:
					data.integer = &in;
					ret =
							access_field_unit(field, byte_offset, UACPI_REGION_OP_READ, data);
					if (uacpi_unlikely_error(ret))
						return ret;
					break;
