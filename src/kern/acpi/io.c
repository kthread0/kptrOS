...
static uacpi_status access_field_unit(
								uacpi_field_unit *field, uacpi_u32 offset, uacpi_region_op op, union uacpi_opregion_io_data data) {
...
uacpi_status uacpi_read_field_unit(uacpi_field_unit *field, void *dst, uacpi_size size, uacpi_data_view *wtr_response) {
	...
	ret = handle_special_field(field, data_view, UACPI_REGION_OP_READ, wtr_response, &did_handle);
	if (did_handle)
		return ret;
	...
	if (field->bit_offset_within_first_byte == 0 && field_byte_length <= field->access_width_bytes) {
		...
		data.integer = &out;
		ret = access_field_unit(field, field->byte_offset, UACPI_REGION_OP_READ, data);
		if (uacpi_unlikely_error(ret))
			return ret;
...
static uacpi_status write_generic_field_unit(uacpi_field_unit *field, const void *src, uacpi_size size) {
	...
	while (bits_left) {
		...
		ret = access_field_unit(field, byte_offset, UACPI_REGION_OP_WRITE, data);
		if (uacpi_unlikely_error(ret))
			return ret;
...
uacpi_status uacpi_write_field_unit(
								uacpi_field_unit *field, const void *src, uacpi_size size, uacpi_data_view *wtr_response) {
	uacpi_status ret;
	uacpi_bool did_handle;
	uacpi_data_view data_view = { 0 };
	...
	ret = handle_special_field(field, data_view, UACPI_REGION_OP_WRITE, wtr_response, &did_handle);
	if (did_handle)
		return ret;
