...
void *
uacpi_kernel_alloc (uacpi_phys_addr addr)
{
	return pmm_alloc (addr);
}
void
uacpi_kernel_free (void *addr)
{
	pmm_free_page (&addr);
}
void
...
uint8_t
uacpi_platform_inb (uint16_t port)
{
	return inb (port);
}
void
uacpi_platform_outb (uint16_t port, uint8_t value)
{
	outb (port, value);
}

uint16_t
uacpi_platform_inw (uint16_t port)
{
	return inb (port);
}
void
uacpi_platform_outw (uint16_t port, uint16_t value)
{
	outb (port, value);
}

uint32_t
uacpi_platform_inl (uint16_t port)
{
	return inb (port);
}
void
uacpi_platform_outl (uint16_t port, uint32_t value)
{
	outb (port, value);
}
