...
dummy_mutex_acquire_release (uacpi_handle mtx)
{
	UACPI_UNUSED (mtx);
...
static void
dump_table_header (uacpi_phys_addr phys_addr, void *hdr)
{
...
static uacpi_status
initialize_from_rxsdt (uacpi_phys_addr rxsdt_addr, uacpi_size entry_size)
{
...
uacpi_status
uacpi_setup_early_table_access (void *temporary_buffer, uacpi_size buffer_size)
{
	uacpi_status ret;
...
void
uacpi_deinitialize_tables (void)
{
	uacpi_size i;
...
static uacpi_u8
table_checksum (void *table, uacpi_size size)
{
...
uacpi_status
uacpi_verify_table_checksum (void *table, uacpi_size size)
{
...
uacpi_bool
uacpi_signatures_match (const void *const lhs, const void *const rhs)
{
	return uacpi_memcmp (lhs, rhs, sizeof (uacpi_object_name)) == 0;
}

uacpi_status
uacpi_check_table_signature (void *table, const uacpi_char *expect)
{
	uacpi_status ret = UACPI_STATUS_OK;
...
static uacpi_status
table_alloc (struct uacpi_installed_table **out_tbl, uacpi_size *out_idx)
{
...
static uacpi_status
get_external_table_header (uacpi_phys_addr phys_addr,
...
static uacpi_status
table_ref_unlocked (struct uacpi_installed_table *tbl)
{
...
static uacpi_status
table_unref_unlocked (struct uacpi_installed_table *tbl)
{
	switch (tbl->reference_count)
		{
		case 0:
...
static uacpi_status
verify_and_install_table (struct acpi_sdt_hdr *hdr, uacpi_phys_addr phys_addr,
													void *virt_addr, enum uacpi_table_origin origin,
													uacpi_table *out_table)
{
	uacpi_status ret;
...
static uacpi_status
table_install_physical_with_origin_unlocked (
		uacpi_phys_addr phys, enum uacpi_table_origin origin,
		const uacpi_char *expected_signature, uacpi_table *out_table)
{
	struct acpi_sdt_hdr hdr;
...
uacpi_status
uacpi_table_install_with_origin (void *virt, enum uacpi_table_origin origin,
																 uacpi_table *out_table)
{
	uacpi_status ret;
...
uacpi_status
uacpi_table_install (void *virt, uacpi_table *out_table)
{
...
uacpi_status
uacpi_table_install_physical (uacpi_phys_addr addr, uacpi_table *out_table)
{
	ENSURE_TABLES_ONLINE ();
...
uacpi_status
uacpi_for_each_table (uacpi_size base_idx, uacpi_table_iteration_callback cb,
											void *user)
{
	uacpi_status ret;
	uacpi_size idx;
		...
		case SEARCH_TYPE_BY_ID:
			{
				...
				if (!uacpi_signatures_match (&id->signature, tbl->hdr.signature))
					return UACPI_ITERATION_DECISION_CONTINUE;
				if (id->oemid[0] != '\0'
						&& uacpi_memcmp (id->oemid, tbl->hdr.oemid, sizeof (id->oemid))
									 != 0)
					return UACPI_ITERATION_DECISION_CONTINUE;

				if (id->oem_table_id[0] != '\0'
						&& uacpi_memcmp (id->oem_table_id, tbl->hdr.oem_table_id,
														 sizeof (id->oem_table_id))
									 != 0)
					return UACPI_ITERATION_DECISION_CONTINUE;
...
#ifndef UACPI_BAREBONES_MODE
uacpi_status
uacpi_table_match (uacpi_size base_idx, uacpi_table_match_callback cb,
									 uacpi_table *out_table)
{
	uacpi_status ret;
...
static uacpi_status
find_table (uacpi_size base_idx, const uacpi_table_identifiers *id,
						uacpi_table *out_table)
{
	uacpi_status ret;
	struct table_search_ctx ctx = { 0 };
...
uacpi_status
uacpi_table_find_by_signature (const uacpi_char *signature_string,
...
static uacpi_status
table_ctl (uacpi_size idx, struct table_ctl_request *req)
{
	uacpi_status ret;
	struct uacpi_installed_table *tbl;
...
#ifndef UACPI_BAREBONES_MODE
uacpi_status
uacpi_table_load_with_cause (uacpi_size idx, enum uacpi_table_load_cause cause)
{
	uacpi_status ret;
	struct table_ctl_request req = {
		.type
		= TABLE_CTL_SET_FLAGS | TABLE_CTL_VALIDATE_CLEAR_FLAGS | TABLE_CTL_GET,
...
uacpi_status
uacpi_table_load (uacpi_size idx)
{
	return uacpi_table_load_with_cause (idx, UACPI_TABLE_LOAD_CAUSE_HOST);
}

void
uacpi_table_mark_as_loaded (uacpi_size idx)
{
	struct table_ctl_request req
...
uacpi_status
uacpi_table_ref (uacpi_table *tbl)
{
...
uacpi_status
uacpi_table_unref (uacpi_table *tbl)
{
...
static void
fadt_ensure_correct_revision (struct acpi_fadt *fadt)
{
...
static void
gas_init_system_io (struct acpi_gas *gas, uacpi_u64 address,
										uacpi_u8 byte_size)
{
	gas->address = address;
...
static void *
fadt_relative (uacpi_size offset)
{
	return ((uacpi_u8 *)&g_uacpi_rt_ctx.fadt) + offset;
}

static void
convert_registers_to_gas (void)
{
	uacpi_size i;
...
#ifndef UACPI_BAREBONES_MODE
static void
split_one_block (struct acpi_gas *src, struct acpi_gas *dst0,
...
static uacpi_status
initialize_fadt (const void *virt)
{
	uacpi_status ret;
	struct acpi_fadt *fadt = &g_uacpi_rt_ctx.fadt;
...
uacpi_status
uacpi_table_fadt (struct acpi_fadt **out_fadt)
{
	ENSURE_TABLES_ONLINE ();
