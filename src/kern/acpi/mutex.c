...
static uacpi_bool
try_acquire_global_lock_from_firmware (uacpi_u32 *lock)
{
...
static uacpi_bool
do_release_global_lock_to_firmware (uacpi_u32 *lock)
{
	uacpi_u32 value, new_value;
...
static uacpi_status
uacpi_acquire_global_lock_from_firmware (void)
{
...
static void
uacpi_release_global_lock_to_firmware (void)
{
	if (!g_uacpi_rt_ctx.has_global_lock)
		return;
...
uacpi_status
uacpi_acquire_native_mutex_with_timeout (uacpi_handle mtx, uacpi_u16 timeout)
{
	uacpi_status ret;
...
uacpi_status
uacpi_acquire_global_lock (uacpi_u16 timeout, uacpi_u32 *out_seq)
{
	uacpi_status ret;
...
uacpi_status
uacpi_release_global_lock (uacpi_u32 seq)
{
	UACPI_ENSURE_INIT_LEVEL_AT_LEAST (UACPI_INIT_LEVEL_SUBSYSTEM_INITIALIZED);
...
uacpi_bool
uacpi_this_thread_owns_aml_mutex (uacpi_mutex *mutex)
{
	uacpi_thread_id id;
...
uacpi_status
uacpi_acquire_aml_mutex (uacpi_mutex *mutex, uacpi_u16 timeout)
{
...
uacpi_status
uacpi_release_aml_mutex (uacpi_mutex *mutex)
{
	if (mutex->depth-- > 1)
		return UACPI_STATUS_OK;
...
uacpi_status
uacpi_recursive_lock_init (struct uacpi_recursive_lock *lock)
{
...
uacpi_status
uacpi_recursive_lock_deinit (struct uacpi_recursive_lock *lock)
{
...
uacpi_status
uacpi_recursive_lock_acquire (struct uacpi_recursive_lock *lock)
{
	uacpi_thread_id this_id;
	uacpi_status ret = UACPI_STATUS_OK;
...
uacpi_status
uacpi_recursive_lock_release (struct uacpi_recursive_lock *lock)
{
	if (lock->depth-- > 1)
		return UACPI_STATUS_OK;
...
uacpi_status
uacpi_rw_lock_init (struct uacpi_rw_lock *lock)
{
...
uacpi_status
uacpi_rw_lock_deinit (struct uacpi_rw_lock *lock)
{
...
uacpi_status
uacpi_rw_lock_read (struct uacpi_rw_lock *lock)
{
	uacpi_status ret;
...
uacpi_status
uacpi_rw_unlock_read (struct uacpi_rw_lock *lock)
{
	uacpi_status ret;
...
uacpi_status
uacpi_rw_lock_write (struct uacpi_rw_lock *lock)
{
	return uacpi_acquire_native_mutex (lock->write_mutex);
}

uacpi_status
uacpi_rw_unlock_write (struct uacpi_rw_lock *lock)
{
